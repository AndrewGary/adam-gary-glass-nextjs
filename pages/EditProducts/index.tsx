          ...
          return (
            <tr key={i} className="border-b border-black text-sm">
              <td className="border border-black text-center">
                <img className="w-16 h-16" src={product.defaultImage} />
              </td>
              <td className="border border-black text-center">
                {product.name}
              </td>
              <td className="pl-1 border border-black text-xs">
                {product.description}
              </td>
              <td className="border border-black text-center">
                ${product.price}
              ...
              <td>
                <div className="flex flex-col space-y-1">
                  <Link
                    ...
                    className="text-center button-styles"
                  >
                    Edit
                  </Link>
                  <button
                    ...
                    onClick={async () => {
                      const resp = await fetch(`/api/products/${product._id}`, {
                        method: "DELETE",
                      });
