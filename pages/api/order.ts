    ...
    switch (req.method) {
        ...
        case 'POST': {
            ...
            const emailResponse = await transporter.sendMail({
              ...
              subject: 'New Order',
              text: 'Help',
              html: `<div>Thanks for your order ${req.body.customer.firstName}</div><div><h4>Shipping Address</h4></div><div>${req.body.customer.firstName} ${req.body.customer.lastName}</div><div>${req.body.customer.address1}</div>${req.body.customer.address2 ? `<div>${req.body.customer.address2}</div>` : ''}<div>${req.body.customer.city}, ${req.body.customer.state} ${req.body.customer.zip}</div><div><span style="font-weight:700">Payment Method: </span>${req.body.paymentMethod}</div><div><span style="font-weight:700">Total</span>: $${req.body.order.total}</div><div><span style="font-weight:700">Please send payment to</span> : AdamsVenmoGoesHere</div><div><br>!!-- Please send payment within the next 48 hours to avoid your items from going back for sale on the site. --!!</div><div><br>You will recieve an email with your tacking number 2 business days after payment is recieved.</div><div>Thank you so much for your support!<br><br></div><div>-Adam Gary Glass<br>adamgaryglass@gmail.com</div><div>(815)508-8556</div>`
            })
